...
def ticket(request):
    if request.user.is_anonymous:
        ...
        if request.method == 'POST':
            ...
            email = "jhdfjgfhj"
            return render(request, 'ticket_form.html', {'form': form, 'email': email})
...
class TicketCreateView(LoginRequiredMixin, generic.CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['email_id'] = self.request.user.email
        return context
...
def resolved_ticket(request):
    tickets = Ticket.objects.filter(
            user=request.user, completed_status=True)
    return render (request, 'closed.html', {'tickets': tickets} )

def ticke_detail(request, pk):
    object = Ticket.objects.filter(ticket_id = pk)
    print(object)
    return render(request, "ticket_detail.html", {"object":object})